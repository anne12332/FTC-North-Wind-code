...
@Autonomous
public class AutoLowJunctionRight extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        Trajectory myTrajectorySpline = drive.trajectoryBuilder(StartPose)
                .lineTo(new Vector2d(-30, -54))
                .build();
        drive.followTrajectory(myTrajectorySpline);

        //тута лифт должен конус на лоу джанкшен поднять
        while (Robot.WB.Lift.getCurrentPosition() <= 1000) {
            Robot.WB.Lift.setPower(0.5);
        }
        drive.turn(Math.toRadians(-45));
        Trajectory myTrajectorySpline2 = drive.trajectoryBuilder(myTrajectorySpline.end())
                .lineTo(new Vector2d(-66, position.getY()))
                .build();
